...
CREATE OR REPLACE PACKAGE BODY compile_pkg IS
   ...
   BEGIN
      ...
      l_catchval := get_type(name_program);
      ...
      ELSE
         ...
         CASE l_catchval
            WHEN 'PACKAGE'   THEN l_stmt := 'ALTER '||l_catchval||' '||name_program||' COMPILE PACKAGE';
            WHEN 'PROCEDURE' THEN l_stmt := 'ALTER '||l_catchval||' '||name_program||' COMPILE';
         END CASE;
